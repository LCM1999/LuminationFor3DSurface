...
float featureSize(const vector<glm::vec3>& vertices) {
	...
	Eigen::Vector3f diagonal(maxX - minX, maxY - minY, maxZ - minZ);
...
// void smooth(const vector<Eigen::Vector3f> normals, vector<Eigen::Vector3f>& smoothed, const float invsigma2 ) {
// 	cout << "Smoothing...\n";
...
// 		Eigen::Vector3f sum = Eigen::Vector3f::Zero(); //constructs to 0,0,0
...
// vtkFloatArray* smoothed
void smooth_parallel(const vector<glm::vec3>& normals, std::vector<glm::vec3>& smoothed, const float invsigma2) {
	...
	*/
	concurrency::parallel_for(size_t(0), normals.size(), [&](size_t i) {
		...
		for (int j = 0; j < normals.size(); j++) {
			float weight = wt(normals[i], normals[j], invsigma2);
			weightSum += weight; //sum of weights
			sum += normals[j] * weight; //add to vertex
		}
		//divide by total weight - can also refactor this to be 1...
		smoothed[i] = sum / weightSum;
		// glm::vec3 smooth = glm::normalize(sum / weightSum);
		// smoothed[i] = smooth;
		});
...
bool writeFile(vector<Eigen::Vector3f> vertices, vector<Eigen::Vector3f> normals, string fileName) {
