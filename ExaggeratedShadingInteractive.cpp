...
int main()
{
   ...
   glfwWindowHint(GLFW_OPENGL_FORWARD_COMPAT, GL_TRUE);
    ...
    YJ poly("F:/LuminationFor3DSurface/1crn_with_New.vtp", "new_normals", "pd1", sigma);
    // YJ poly("F:/LuminationFor3DSurface/airplane.vtp", "Normals", "pressure", sigma);
    ...
    glm::vec3 cen = center(poly.vertices); 
    cout << "Center of model : " << cen.x << ", " << cen.y << ", " << cen.z << "\n";
    ...
    //Load Shaders
    GLuint raster = loadShader("F:/LuminationFor3DSurface/shaders/gBuffer.vs","F:/LuminationFor3DSurface/shaders/gBuffer.fs");
    GLuint cosine = loadShader("F:/LuminationFor3DSurface/shaders/cosine.vs", "F:/LuminationFor3DSurface/shaders/cosine.fs");
    ...
    //    }
        ...
        glUseProgram(raster);
        glUniformMatrix4fv(glGetUniformLocation(raster, "model"), 1, GL_FALSE, &model[0][0]);
        glUniformMatrix4fv(glGetUniformLocation(raster, "view"), 1, GL_FALSE, &view[0][0]);
        glUniformMatrix4fv(glGetUniformLocation(raster, "projection"), 1, GL_FALSE, &projection[0][0]);
        glUseProgram(0);    
        // geometry pass
        glBindFramebuffer(GL_FRAMEBUFFER, gBuffer);
        //z buffer
        glEnable(GL_DEPTH_TEST);
        glDepthFunc(GL_LESS);  // 默认值，确保未被其他代码修改
        glDepthMask(GL_TRUE);  // 允许写入深度缓冲  
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        poly.geomRender(raster);
        ...
        if (needWriteDepth)
        {
            ...
            std::vector<GLfloat> depthArr(arrLen);
            glBindFramebuffer(GL_FRAMEBUFFER, gBuffer);
            glReadBuffer(GL_NONE);
            glReadPixels(0, 0, WIDTH, HEIGHT, GL_DEPTH_COMPONENT, GL_FLOAT, depthArr.data());
            glBindFramebuffer(GL_FRAMEBUFFER, 0);
        ...
        glUseProgram(cosine);
        glUniform3fv(glGetUniformLocation(cosine, "viewPos"), 1, &viewPos[0]);
        glUniform3f(glGetUniformLocation(cosine, "light.position"), lightPos.x, lightPos.y, lightPos.z);
        glUniform3f(glGetUniformLocation(cosine, "light.diffuse"), lightDiffuse.x, lightDiffuse.y, lightDiffuse.z);
        glUniform3f(glGetUniformLocation(cosine, "highColor"),textureHigh.x, textureHigh.y, textureHigh.z);
        glUniform3f(glGetUniformLocation(cosine, "lowColor"), textureLow.x, textureLow.y, textureLow.z);
        glUniform1f(glGetUniformLocation(cosine, "ambientFactor"), ambient);
        glUniform1f(glGetUniformLocation(cosine, "phi"), phi);

        // Bind G-buffer textures
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, gPosition);
        glUniform1i(glGetUniformLocation(cosine, "gPosition"), 0);

        glActiveTexture(GL_TEXTURE1);
        glBindTexture(GL_TEXTURE_2D, gNormal);
        glUniform1i(glGetUniformLocation(cosine, "gNormal"), 1);

        glActiveTexture(GL_TEXTURE2);
        glBindTexture(GL_TEXTURE_2D, gAlbedoSpec);
        glUniform1i(glGetUniformLocation(cosine, "gAlbedoSpec"), 2);
        
        glActiveTexture(GL_TEXTURE3);
        glBindTexture(GL_TEXTURE_2D, gDepth);
        glUniform1i(glGetUniformLocation(cosine, "gDepth"), 3);
        
        glUseProgram(0);
        poly.quadRender(cosine);
